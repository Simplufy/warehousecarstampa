...
const defaultMake = {
  ...
  buyingTip: 'When shopping for a quality used vehicle, always check the service history, take a thorough test drive, and have the vehicle inspected by a trusted mechanic. At Tampa Autohaus, every vehicle on our lot has been inspected to meet our quality standards.'
};
...
function getMakeContent(make, stats, cars) {
  ...
  const introText = `<p>Looking for a used ${make} in Tampa, FL? Tampa Autohaus currently has <strong>${count} pre-owned ${make} vehicle${count !== 1 ? 's' : ''}</strong>${count > 0 ? ` starting at just <strong>$${fmtPrice(stats.min_price)}</strong>` : ''}. ${make} is known for ${data.reliability}, making it one of the smartest choices in the used car market.</p>
<p>At Tampa Autohaus, every ${make} on our lot has been thoroughly inspected for quality and reliability. We believe in transparent pricing with no hidden fees — the price you see is the price you pay. Whether you're a first-time buyer or upgrading your current ride, our team is here to help you find the perfect ${make} without the pressure.</p>
<p>${data.buyingTip}</p>
${count > 0 && bodyStyles.length > 0 ? `<p>Our current ${make} inventory includes ${bodyStyles.join(', ').toLowerCase()} body styles${years.length > 1 ? ` from model years ${years[0]} through ${years[years.length - 1]}` : ''}. Browse below and contact us to schedule a test drive — we're conveniently located in Tampa, FL and serve drivers from Brandon, St. Petersburg, Wesley Chapel, Clearwater, and across the Tampa Bay area.</p>` : `<p>While our inventory changes frequently, we regularly stock ${make} vehicles. Contact us today and we'll help you find exactly what you're looking for — or notify you as soon as one arrives on our lot.</p>`}`;
  ...
  const faqs = [
    { q: `Why buy a used ${make} in Tampa?`, a: `${make} vehicles offer ${data.reliability}. Buying used saves you thousands compared to new, and Tampa's large used car market means great selection. At Tampa Autohaus, we hand-pick quality ${make} vehicles so you can buy with confidence.` },
    { q: `How much does a used ${make} cost at Tampa Autohaus?`, a: count > 0 ? `Our used ${make} inventory currently ranges from $${fmtPrice(stats.min_price)} to $${fmtPrice(stats.max_price)}, with an average price of $${fmtPrice(stats.avg_price)}. We offer competitive pricing and financing options to fit your budget.` : `Pricing varies by model, year, and condition. Contact us for current ${make} availability and pricing. We offer competitive prices and flexible financing options.` },
  ...
  return {
    title: `Used ${make} for Sale in Tampa FL | Tampa Autohaus`,
    metaDescription: count > 0 ? `Browse ${count} used ${make} vehicle${count !== 1 ? 's' : ''} for sale at Tampa Autohaus. Starting at $${fmtPrice(stats.min_price)}. ${data.tagline.charAt(0).toUpperCase() + data.tagline.slice(1)}.` : `Shop used ${make} vehicles at Tampa Autohaus in Tampa, FL. ${data.tagline.charAt(0).toUpperCase() + data.tagline.slice(1)}. Quality pre-owned cars at fair prices.`,
...
function getMakeModelContent(make, model, stats, cars) {
  ...
  let introText;
  if (mData) {
    introText = `<p>Find the perfect used ${make} ${model} right here at Tampa Autohaus in Tampa, FL. ${count > 0 ? `We currently have <strong>${count} ${make} ${model}${count !== 1 ? 's' : ''}</strong> in stock${count > 0 ? `, priced from <strong>$${fmtPrice(priceMin)}</strong>${priceMax !== priceMin ? ` to <strong>$${fmtPrice(priceMax)}</strong>` : ''}` : ''}.` : `While we don't currently have a ${make} ${model} in stock, our inventory changes frequently.`}</p>
...
<p>${data.buyingTip}</p>
<p>Every vehicle at Tampa Autohaus goes through our quality inspection process before it reaches our lot. We stand behind what we sell and offer transparent, no-haggle pricing. Stop by our Tampa location or contact us to schedule a test drive today.</p>`;
  } else {
    introText = `<p>Find the perfect used ${make} ${model} right here at Tampa Autohaus in Tampa, FL. ${count > 0 ? `We currently have <strong>${count} ${make} ${model}${count !== 1 ? 's' : ''}</strong> in stock${count > 0 ? `, priced from <strong>$${fmtPrice(priceMin)}</strong>${priceMax !== priceMin ? ` to <strong>$${fmtPrice(priceMax)}</strong>` : ''}` : ''}.` : `While we don't currently have a ${make} ${model} in stock, our inventory changes frequently.`}</p>
...
<p>Every vehicle at Tampa Autohaus goes through our quality inspection process before it reaches our lot. We stand behind what we sell and offer transparent, no-haggle pricing. Stop by our Tampa location or contact us to schedule a test drive today.</p>`;
  ...
  if (mData) {
    faqs = [
      ...
      { q: `Can I test drive a ${make} ${model} at Tampa Autohaus?`, a: `Of course! Just give us a call or use our contact form to schedule a test drive. Walk-ins are also welcome during business hours, Monday through Saturday.` }
    ];
  } else {
    faqs = [
      { q: `How much is a used ${make} ${model} in Tampa?`, a: count > 0 ? `At Tampa Autohaus, our used ${make} ${model} inventory is priced from $${fmtPrice(priceMin)} to $${fmtPrice(priceMax)}. Pricing depends on year, mileage, and condition.` : `Pricing varies by year, mileage, and condition. Contact us for current availability and pricing on the ${make} ${model}.` },
  ...
  return {
    title: `Used ${make} ${model} for Sale in Tampa FL | Tampa Autohaus`,
    metaDescription: count > 0 ? `${count} used ${make} ${model}${count !== 1 ? 's' : ''} for sale at Tampa Autohaus starting at $${fmtPrice(priceMin)}. Quality pre-owned vehicles in Tampa, FL.` : `Shop used ${make} ${model} vehicles at Tampa Autohaus in Tampa, FL. Quality inspected, fair pricing, financing available.`,
...
const defaultBodyStyle = {
  ...
  tip: 'No matter what body style you prefer, Tampa Autohaus has options to fit your lifestyle and budget.'
};
...
function getBodyStyleContent(bodyStyle, stats, cars) {
  ...
  const introText = `<p>Shopping for a used ${bodyStyle.toLowerCase()} in Tampa, FL? You're in the right place. Tampa Autohaus has <strong>${count} pre-owned ${data.plural.toLowerCase()}</strong>${count > 0 ? ` starting at just <strong>$${fmtPrice(stats.min_price)}</strong>` : ''}, offering ${data.description}.</p>
...
${makes.length > 0 ? `<p>Our current ${bodyStyle.toLowerCase()} selection includes popular makes like <strong>${makes.join(', ')}</strong>. Each vehicle has been inspected for quality and is priced competitively with no hidden fees. Browse our inventory below, and contact us to schedule a test drive at our Tampa location.</p>` : `<p>Our inventory changes regularly — contact us today to find out what ${data.plural.toLowerCase()} we have available or to be notified when new ones arrive.</p>`}`;
  ...
  const faqs = [
    { q: `What are the best used ${data.plural.toLowerCase()} to buy in Tampa?`, a: `It depends on your needs and budget. ${makes.length > 0 ? `Popular choices in our inventory include ${makes.join(', ')} ${data.plural.toLowerCase()}.` : ''} At Tampa Autohaus, we can help you find the perfect ${bodyStyle.toLowerCase()} for your lifestyle and budget.` },
    { q: `How much do used ${data.plural.toLowerCase()} cost at Tampa Autohaus?`, a: count > 0 ? `Our used ${data.plural.toLowerCase()} range from $${fmtPrice(stats.min_price)} to $${fmtPrice(stats.max_price)}. We offer transparent pricing and financing options.` : `Pricing varies by make, model, year, and condition. Contact us for current availability.` },
  ...
  return {
    title: `Used ${data.plural} for Sale in Tampa FL | Tampa Autohaus`,
    metaDescription: count > 0 ? `Browse ${count} used ${data.plural.toLowerCase()} for sale at Tampa Autohaus starting at $${fmtPrice(stats.min_price)}. ${data.description.charAt(0).toUpperCase() + data.description.slice(1)}.` : `Shop used ${data.plural.toLowerCase()} at Tampa Autohaus. ${data.description.charAt(0).toUpperCase() + data.description.slice(1)}.`,
...
function getPriceRangeContent(maxPrice, stats, cars) {
  ...
  const introText = `<p>Looking for quality used cars under $${fmtMax} in Tampa? Tampa Autohaus has <strong>${count} affordable vehicle${count !== 1 ? 's' : ''}</strong> priced under $${fmtMax}${count > 0 ? `, starting at just <strong>$${fmtPrice(stats.min_price)}</strong>` : ''}.</p>
...
<p>At Tampa Autohaus, we specialize in helping Tampa drivers find reliable transportation that fits their budget. We offer financing options including programs for first-time buyers and those rebuilding credit. Browse our selection below or contact us to find the perfect vehicle under $${fmtMax}.</p>`;
  ...
  const faqs = [
    ...
    { q: `Are cheap used cars reliable?`, a: `Absolutely — price doesn't always equal quality. Many used cars under $${fmtMax} have years of reliable service left in them, especially well-maintained Toyota, Honda, and Hyundai models. At Tampa Autohaus, every vehicle passes our inspection before it hits the lot.` }
  ];

  return {
    title: `Used Cars Under $${fmtMax} in Tampa FL | Tampa Autohaus`,
    metaDescription: count > 0 ? `${count} quality used cars under $${fmtMax} in Tampa, FL. Starting at $${fmtPrice(stats.min_price)}. Financing available at Tampa Autohaus.` : `Shop affordable used cars under $${fmtMax} at Tampa Autohaus in Tampa, FL. Quality vehicles, transparent pricing, financing available.`,
...
function getYearContent(year, stats, cars) {
  ...
  const introText = `<p>Browse <strong>${count} used ${year} vehicle${count !== 1 ? 's' : ''}</strong> for sale at Tampa Autohaus in Tampa, FL${count > 0 ? `. Prices start at <strong>$${fmtPrice(stats.min_price)}</strong>` : ''}.</p>
...
<p>Whether you're looking for a fuel-efficient sedan for your Tampa commute or a spacious SUV for family adventures, Tampa Autohaus has ${year} options to fit your needs. Contact us to schedule a test drive today.</p>`;
  ...
  return {
    title: `${year} Used Cars for Sale in Tampa FL | Tampa Autohaus`,
    metaDescription: count > 0 ? `${count} used ${year} vehicles for sale in Tampa, FL starting at $${fmtPrice(stats.min_price)}. Quality inspected cars at Tampa Autohaus.` : `Shop ${year} used cars at Tampa Autohaus in Tampa, FL. Quality inspected vehicles with transparent pricing.`,
